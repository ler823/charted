...
export default function EditListFriends({ isVisible, onClose, onSave, listId, pinsInList, setPinsToAdd }: Props) {
    ...
    const getUserFriends = async () => {
      ...
      `)
        ...
        const userId = 4
        const friendIdList = data?.map(({ requester_id, target_id }) => requester_id == userId ? Number(target_id) : Number(requester_id)) ?? []
        ...
        setFriends(friendData?.map((item) => ({ userId: item.user_id, name: item.username, location: item.location, photoKey: item.photos?.key ?? "" })))
